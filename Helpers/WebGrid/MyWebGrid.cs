...
using BootstrapHtmlHelper.Internal.Web.Utils;
using BootstrapHtmlHelper.Helpers.Resources;
...
namespace BootstrapHtmlHelper.Helpers
{
    public class MyWebGrid
    {
        ...
        // NOTE: MyWebGrid uses an IEnumerable<dynamic> data source instead of IEnumerable<T> to avoid generics in the syntax.
        internal MyWebGrid(
       ...
       public  AjaxHelper AjaxHelper { get;  set; }
        ...
        public MyWebGrid Bind(IEnumerable<dynamic> source, IEnumerable<string> columnNames = null, bool autoSortAndPage = true, int rowCount = -1)
        {
            if (_dataSourceBound)
            {
        ...
        // Should we keep this no-op API for improved MyWebGrid syntax? Alternatives are:
        ...
        public IHtmlString GetHtml(
            ...
            MyWebGridPagerModes mode = MyWebGridPagerModes.NextPrevious | MyWebGridPagerModes.Numeric,
        ...
        public string GetSortUrl(string column)
        {
            ...
            if (IsAjaxEnabled)
            {
                ...
                AjaxHelper.ActionLink(
                    linkText: GetSortDirectionString(sortDir),
                    actionName: ActionName,
                    controllerName: ControllerName,
                    ajaxOptions: AjaxOptions).ToHtmlString();
            }
            */
        ...
        /// <param name="lastText">Text for link to last page.</param>
        /// <param name="numericLinksCount">Number of numeric links that should display.</param>
        /// <param name="explicitlyCalled">The Pager can be explicitly called by the public API or is called by the MyWebGrid when no footer is provided.
            ...
            bool explicitlyCalled)
        {
            ...
            if (!ModeEnabled(mode, MyWebGridPagerModes.FirstLast) && (firstText != null))
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                          HelpersResources.WebGrid_PagerModeMustBeEnabled, "FirstLast"), "firstText");
            }
            if (!ModeEnabled(mode, MyWebGridPagerModes.NextPrevious) && (previousText != null))
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                          HelpersResources.WebGrid_PagerModeMustBeEnabled, "NextPrevious"), "previousText");
            }
            if (!ModeEnabled(mode, MyWebGridPagerModes.NextPrevious) && (nextText != null))
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                          HelpersResources.WebGrid_PagerModeMustBeEnabled, "NextPrevious"), "nextText");
            }
            if (!ModeEnabled(mode, MyWebGridPagerModes.FirstLast) && (lastText != null))
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
            ...
            string paginationStyle = null)
        {
            ...
            return PagerList( mode, firstText, previousText, nextText, lastText, numericLinksCount, paginationStyle, explicitlyCalled: true);
        }
            ...
            bool explicitlyCalled)
        {
            ...
            if (this.TotalRowCount <= this.PageCount)
            {
            ...
            if (ModeEnabled(mode, MyWebGridPagerModes.FirstLast))
            {
                if (String.IsNullOrEmpty(firstText))
                {
                    firstText = "First";
                }
                var url = this.GetPageUrl(0);
                var part = new TagBuilder("li")
                {
                    InnerHtml = this.GridLink(0, firstText).ToHtmlString()
                };
            ...
            if (ModeEnabled(mode, MyWebGridPagerModes.NextPrevious))
            {
                ...
                var part = new TagBuilder("li")
                {
                    InnerHtml = GridLink(page, previousText).ToHtmlString()
                };
            ...
            if (ModeEnabled(mode, MyWebGridPagerModes.Numeric) && (totalPages > 1))
            {
                ...
                for (int i = first; i <= last; i++)
                {
                    ...
                    var part = new TagBuilder("li")
                    {
                        InnerHtml = GridLink(i, pageText).ToHtmlString()
                    };
            ...
            if (ModeEnabled(mode, MyWebGridPagerModes.NextPrevious))
            {
                ...
                var part = new TagBuilder("li")
                {
                    InnerHtml = GridLink(page, nextText).ToHtmlString()
                };
            ...
            if (ModeEnabled(mode, MyWebGridPagerModes.FirstLast))
            {
                ...
                var part = new TagBuilder("li")
                {
                    InnerHtml = this.GridLink(lastPage, lastText).ToHtmlString()
                };
            ...
            var html = "";
            if (explicitlyCalled && this.IsAjaxEnabled)
            {
                var span = new TagBuilder("span");
                //span.MergeAttribute("data-swhgajax", "true");
                //span.MergeAttribute("data-swhgcontainer", this.AjaxUpdateContainerId);
                //span.MergeAttribute("data-swhgcallback", this.AjaxUpdateCallback);
        ...
        public  IHtmlString GridLink(int page,string text)
        {
            string url = this.GetPageUrl(page);
            ...
            if (this.IsAjaxEnabled)
            {
                ...
                builder.MergeAttribute("data-ajax-mode", this.AjaxOptions.InsertionModeString());

                builder.MergeAttribute("data-ajax-update", "#" + this.AjaxUpdateContainerId);
            }
            ...
            object htmlAttributes = null)
        {
            ...
            // In order of precedence, the parameters that affect the visibility of columns in MyWebGrid - 
        ...
        public MyWebGrid AddSorter<TElement, TProperty>(string columnName, Expression<Func<TElement, TProperty>> keySelector)
        {
        ...
        internal string GetPath(NameValueCollection queryString, params string[] exclusions)
        {
            ...
            StringBuilder sb = new StringBuilder(
                UrlHelper.GenerateUrl(null,ActionName,
                string.IsNullOrEmpty(this.ControllerName)? HttpContext.Request.RequestContext.RouteData.Values["controller"].ToString().ToString() : this.ControllerName,                
                null,
                RouteTable.Routes,
                HttpContext.Request.RequestContext,false
                )
        ...
        private static bool ModeEnabled(MyWebGridPagerModes mode, MyWebGridPagerModes modeCheck)
        {
